...
describe CloudConductorPattern::EventHandler do
  before do
    ...
    allow(CloudConductorPattern::PatternLogger).to receive(:logger).with(
      '/opt/cloudconductor/patterns/tomcat_pattern/logs/event-handler.log'
  ...
  describe '#initialize' do
    it 'creates and returns new instance' do
      ...
      expect(event_handler.pattern_name).to eq('tomcat_pattern')
      expect(event_handler.pattern_dir).to eq('/opt/cloudconductor/patterns/tomcat_pattern')
  ...
  describe '#execute_chef' do
    it 'executes chef' do
      allow(File).to receive(:exist?).with(
        '/opt/cloudconductor/patterns/tomcat_pattern/roles/web_setup.json'
      ).and_return(true)
      allow(File).to receive(:exist?).with(
        '/opt/cloudconductor/patterns/tomcat_pattern/roles/ap_setup.json'
      ).and_return(false)
      allow(File).to receive(:exist?).with(
        '/opt/cloudconductor/patterns/tomcat_pattern/roles/db_setup.json'
      ).and_return(true)
      allow(File).to receive(:exist?).with(
        '/opt/cloudconductor/patterns/tomcat_pattern/roles/all_setup.json'
      ).and_return(false)
      event_handler = CloudConductorPattern::EventHandler.new('web,ap,db')
  ...
  describe '#execute_serverspec' do
    it 'executes serverspec in configure phase' do
      ...
      allow(File).to receive(:exist?).with(
        '/opt/cloudconductor/patterns/tomcat_pattern/serverspec/spec/web/web_configure_spec.rb'
      ).and_return(true)
      allow(File).to receive(:exist?).with(
        '/opt/cloudconductor/patterns/tomcat_pattern/serverspec/spec/ap/ap_configure_spec.rb'
      ).and_return(false)
      allow(File).to receive(:exist?).with(
        '/opt/cloudconductor/patterns/tomcat_pattern/serverspec/spec/db/db_configure_spec.rb'
      ).and_return(true)
      allow(File).to receive(:exist?).with(
        '/opt/cloudconductor/patterns/tomcat_pattern/serverspec/spec/all/all_configure_spec.rb'
      ).and_return(false)
      event_handler.send(:execute_serverspec)
      expect(event_handler.command).to eq(
        [
          'cd /opt/cloudconductor/patterns/tomcat_pattern/serverspec; rake spec[web,configure]',
          'cd /opt/cloudconductor/patterns/tomcat_pattern/serverspec; rake spec[db,configure]'
        ]
      )
    end
    ...
    it 'executes serverspec in deploy phase' do
      ...
      allow(File).to receive(:exist?).with(
        '/opt/cloudconductor/patterns/tomcat_pattern/serverspec/spec/web/web_deploy_spec.rb'
      ).and_return(true)
      allow(File).to receive(:exist?).with(
        '/opt/cloudconductor/patterns/tomcat_pattern/serverspec/spec/ap/ap_deploy_spec.rb'
      ).and_return(false)
      allow(File).to receive(:exist?).with(
        '/opt/cloudconductor/patterns/tomcat_pattern/serverspec/spec/db/db_deploy_spec.rb'
      ).and_return(true)
      allow(File).to receive(:exist?).with(
        '/opt/cloudconductor/patterns/tomcat_pattern/serverspec/spec/all/all_deploy_spec.rb'
      ).and_return(false)
      event_handler.send(:execute_serverspec)
      expect(event_handler.command).to eq(
        [
          'cd /opt/cloudconductor/patterns/tomcat_pattern/serverspec; rake spec[web,configure]',
          'cd /opt/cloudconductor/patterns/tomcat_pattern/serverspec; rake spec[web,deploy]',
          'cd /opt/cloudconductor/patterns/tomcat_pattern/serverspec; rake spec[db,configure]',
          'cd /opt/cloudconductor/patterns/tomcat_pattern/serverspec; rake spec[db,deploy]'
        ]
      )
    end
  end
  ...
  describe '#create_chefsolo_config_file' do
    it 'creates config file' do
      ...
      allow(dummy_file).to receive(:write).with("role_path '/opt/cloudconductor/patterns/tomcat_pattern/roles'\n")
      allow(dummy_file).to receive(:write).with("log_level :info\n")
      allow(dummy_file).to receive(:write).with("log_location '/opt/cloudconductor/patterns/tomcat_pattern/logs/tomcat_pattern_web_chef-solo.log'\n")
      allow(dummy_file).to receive(:write).with("file_cache_path '/opt/cloudconductor/patterns/tomcat_pattern/tmp/cache'\n")
      allow(dummy_file).to receive(:write).with("cookbook_path ['/opt/cloudconductor/patterns/tomcat_pattern/cookbooks', '/opt/cloudconductor/patterns/tomcat_pattern/site-cookbooks']\n")
      event_handler = CloudConductorPattern::EventHandler.new('web,ap,db')
      allow(File).to receive(:open).with(
        '/opt/cloudconductor/patterns/tomcat_pattern/solo.rb',
        'w'
  ...
  describe '#create_chefsolo_node_file' do
    it 'creates node file in case of setup' do
      ...
      allow(File).to receive(:write).with(
        '/opt/cloudconductor/patterns/tomcat_pattern/node.json',
    ...
    it 'creates node file in case of not setup' do
      ...
      expect(result).to eq('/opt/cloudconductor/patterns/tomcat_pattern/node.json')
    end
  end
  ...
  describe '#run_chefsolo' do
    it 'runs chef-solo' do
      ...
      expect(event_handler.command).to eq(
        [
          'cd /opt/cloudconductor/patterns/tomcat_pattern; berks vendor ./cookbooks',
          'chef-solo -c /opt/cloudconductor/patterns/tomcat_pattern/solo.rb -j /opt/cloudconductor/patterns/tomcat_pattern/node.json'
        ]
      )
    end
  end
