...
def execute_serverspec(roles):
  ...
  for role in roles:
    for event in event_list:
      spec_file = os.path.join( spec_dir, role, role + '_' + event + '_spec.rb' )
      if os.path.exists( spec_file ):
        logging.info( 'execute serverspec with [%s].', spec_file )

        cmd = 'cd ' + test_root + '; rake spec[' + role + ',' + event + ']'
        result = system( cmd )
        ...
        logging.info( 'spec file [%s] does not exist. skipped.', spec_file )
...
def create_chefsolo_config_file(role):
  ...
  log_file = os.path.join( log_dir(), pattern_name + '_' + role + '_chef-solo.log' )
  ...
  fo.write( "cookbook_path ['" + cookbooks_dir + "', '"+ site_cookbooks_dir + "']" )
...
def create_chefsolo_node_file(role, event):
  ...
  if event != 'setup':
    data = deep_merge( parameters, pattern( parameters, pattern_name )['user_attributes'] )
